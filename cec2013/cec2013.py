...
from cec2013.cfunction import *
from cec2013.CF1 import *
from cec2013.CF2 import *
from cec2013.CF3 import *
from cec2013.CF4 import *
...
class CEC2013(object):
    ...
    def evaluate(self, x):
        ...
        assert x_.shape[1] == self.get_dimension()
        if self.__nfunc_ > 0 and self.__nfunc_ < 11:
            return self.__f_(x_)
        else:
...
def how_many_goptima(pop, f, accuracy, archive_val_copy):
    # pop: NP, D
    NP, D = pop.shape[0], pop.shape[1]

    # Evaluate population
    fits = np.zeros(NP)
    fits = f.evaluate(pop.copy())
    assert ((fits == archive_val_copy).all())
